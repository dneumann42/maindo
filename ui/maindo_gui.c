...
void mdo_button(Mdo_ButtonConfig cfg) {
  Clay_BorderElementConfig border_cfg = {};
  Clay_Color bg_color = cfg.color;

  if (cfg.variant == BUTTON_OUTLINED) {
    border_cfg = BORDER_CONFIG;
    bg_color = (Clay_Color){};
  }
  ...
  CLAY({.layout = {.padding = CLAY_PADDING_ALL(8)},
    ...
    CLAY_TEXT(
        ...
        CLAY_TEXT_CONFIG({.fontSize = 16,
                          .textColor = mdo_theme_color(COLOR_TEXT_PRIMARY)}));
  }
}
...
void HandleInputStateInteraction(Clay_ElementId element_info,
                                 Clay_PointerData pointer_info,
                                 intptr_t user_data) {
  TextInputState *button = (TextInputState *)user_data;
  if (pointer_info.state == CLAY_POINTER_DATA_PRESSED_THIS_FRAME) {
    button->editing = true;
  }
}
...
void mdo_text_input(Mdo_TextInputConfig cfg) {
  CLAY({.userData = cfg.state,
        .backgroundColor = mdo_theme_color(COLOR_FOREGROUND),
        .border = BORDER_CONFIG,
        .cornerRadius = CORNER_RADIUS,
        .layout = {.sizing = {.width = CLAY_SIZING_FIXED(256),
                              .height = CLAY_SIZING_FIXED(32)}}}) {
    Clay_OnHover(HandleInputStateInteraction, (intptr_t)cfg.state);
    if (cfg.placeholder.chars && cfg.state->string.length == 0) {
      CLAY_TEXT(
          cfg.placeholder,
          CLAY_TEXT_CONFIG({.fontSize = 16,
                            .textColor = mdo_theme_color(COLOR_TEXT_PRIMARY)}));
    } else {
      CLAY_TEXT(cfg.state->string,
                CLAY_TEXT_CONFIG(
                    {.fontSize = 16,
                     .textColor = mdo_theme_color(COLOR_TEXT_SECONDARY)}));
    }
  }
}
...
Clay_RenderCommandArray Demo_CreateLayout(void) {
  ...
  *state = (ButtonState){.on_click = on_click};
  ...
  TextInputState *input_state = SDL_calloc(1, sizeof(TextInputState));
  input_state->string = CLAY_STRING("");

  // Toolbar
  CLAY({.layout = {.layoutDirection = CLAY_TOP_TO_BOTTOM, .childGap = 8.0}}) {
    CLAY({.layout = {.layoutDirection = CLAY_LEFT_TO_RIGHT, .childGap = 8.0}}) {
      mdo_button((Mdo_ButtonConfig){.state = state,
                                    .text = CLAY_STRING("New Entry"),
                                    .color = mdo_theme_color(COLOR_PRIMARY),
                                    .variant = BUTTON_FILLED});
      mdo_button((Mdo_ButtonConfig){.state = state,
                                    .text = CLAY_STRING("New Library"),
                                    .color = mdo_theme_color(COLOR_PRIMARY),
    ...
    mdo_text_input((Mdo_TextInputConfig){.placeholder = CLAY_STRING("Name"),
                                         .state = input_state});
  }
